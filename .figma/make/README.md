...
These scripts run a fast Shopware setup for Figma Make. They target the Administration watcher at `http://localhost:9173` and intentionally skip full storefront and production asset builds.
...
- `FIGMA_MAKE_URL` defaults to `http://localhost:9173`.
...
If Administration text renders as snippet keys such as `sw-login.index.headlineMain`, the watcher is usually running but its API proxy cannot reach Shopware. In this Docker setup, the host-facing URL is `http://localhost:9000`, while the watcher runs inside the container and must proxy to `http://localhost:8000`.
